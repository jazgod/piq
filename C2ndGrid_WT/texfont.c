...
GLuint
txfEstablishTexture(struct InstanceData* id, 
  TexFont * txf,
  GLuint texobj,
  GLboolean setupMipmaps)
{
  if (txf->texobj == 0) {
    if (texobj == 0) {
	  ...
	  vo_log_info(id->m_pfVoLogFile, __FUNCTION__, "(0x%X) Generated Textfont Texture: %d.\n", GetCurrentThreadId(), txf->texobj); 
#else
